...
describe('checkFindings', () => {
  ...
  // output of a version-pinned git dependency (@arcade/shared): its dist/ is
  // gitignored and regenerated on every install, so its line numbers move under the
  // audit each time the library is re-pinned or rebuilt. A citation the checker
  // cannot trust is worthless — the whole point of the ours-side check is that a
  // cited line can be re-opened and byte-compared. So the category error is made
  // impossible: `ours` means OUR source — a tracked file in this repo — and a
  // dependency's build output is rejected on sight.
  it('rejects an `ours` citation into node_modules — a rebuilt artifact is not our source', () => {
    // The fixture cites node_modules with its CURRENT, byte-exact line, so the
    // verbatim check PASSES. That is deliberate: a naive assertion (cite a stale
    // line, expect "an error") passes today for the wrong reason — the checker would
    // emit "ours node_modules/...:46 does not match verbatim", and that message
    // CONTAINS "node_modules" simply because the path is in it. Such a test would be
    // vacuous: green whether or not the rule exists. By citing the line correctly, the
    // ONLY thing that can produce an error is a rule that rejects the PATH.
