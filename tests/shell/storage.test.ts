...
//
// RED-phase suite for Story 8-6 (Wave 4 — framing), Part B: the SHELL
// persistence seam (src/shell/storage.ts), localStorage-backed high scores.
// This MIRRORS tempest's tests/shell/storage.test.ts (the explicit reuse mandate)
// adapted to star-wars: the storage key is `star-wars-high-scores` and an entry
// records the **wave** reached (not tempest's `level`).
//
...
// Read the shell source as text (Vite `?raw`) for the signature-level AC below.
...
// read this same key), so assert the literal rather than re-importing it.
const STORAGE_KEY = 'star-wars-high-scores'
...
const SAMPLE_TABLE: HighScoreTable = [
  { name: 'AAA', score: 50000, wave: 4, date: '2026-06-26T00:00:00.000Z' },
  { name: 'BOB', score: 30000, wave: 3 },
  { name: 'CDE', score: 10000, wave: 1, date: '2026-06-25T12:00:00.000Z' },
]
...
describe('saveHighScores', () => {
  it('writes the table as JSON under the star-wars-high-scores key', () => {
...
describe('loadHighScores — per-entry validation guard', () => {
  ...
  it('keeps only the well-formed rows from a mixed array, preserving order and date', () => {
    const mixed: unknown[] = [
      { name: 'AAA', score: 50000, wave: 4, date: '2026-06-26T00:00:00.000Z' }, // keep
      {}, // drop
